$.fn.dropdown = function(prov, city, area){
	var provToggle = prov+' .dropdown-toggle'
	var cityToggle = city+' .dropdown-toggle'
	var areaToggle = area+' .dropdown-toggle'
	var provLi = prov+' .dropdown-menu li'
	var cityLi = city+' .dropdown-menu li'
	var areaLi = area+' .dropdown-menu li'
	$(".dropdown-inner").on("click", function(){
		if ($(this).find(".dropdown-menu").css("display") == "none") {
			$(".dropdown-menu").slideUp(300);
			$(this).find(".dropdown-menu").slideDown(300);
			$(this).find(".dropdown-toggle").addClass("select");
		}else{
			$(this).find(".dropdown-menu").slideUp(300);
			$(this).find(".dropdown-toggle").removeClass("select");
		}
	});
	var provDefault = provinceList;//默认省份
	var cityDefault = provinceList[0].cityList;//默认城市
	var areaDefault = provinceList[0].cityList[0].areaList;//默认地区
	
	$(provToggle).text(provinceList[0].name);
	$(cityToggle).text(provinceList[0].cityList[0].name);
	$(areaToggle).text(provinceList[0].cityList[0].areaList[0]);
	
	for(a = 0; a < provDefault.length; a++){
		$(prov).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ provDefault[a].name +"'>"+ provDefault[a].name +"</a></li>");
	}
	for(b = 0; b < cityDefault.length; b++){
		$(city).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ cityDefault[b].name +"'>"+ cityDefault[b].name +"</a></li>");
	}
	for(c = 0; c < areaDefault.length; c++){
		$(area).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ areaDefault[c].name +"'>"+ areaDefault[c] +"</a></li>");
	}



	$(provLi).click(function(){//省份的li单击事件
		$(prov).find('.dropdown-toggle').text($(this).text());
		$(prov).find('.dropdown-toggle').attr('title', $(this).text());
		var provIndex = $(this).index();
		
		var cityList = provinceList[provIndex].cityList;
		$(city).find('.dropdown-menu').text('');
		for(i = 0; i < cityList.length; i++){
			$(city).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ cityList[i].name +"'>"+ cityList[i].name +"</a></li>");
		}

		var areaList = provinceList[provIndex].cityList[0].areaList;
		$(area).find('.dropdown-menu').text('');
		for(j = 0; j < areaList.length; j++){
			$(area).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ areaList[j] +"'>"+ areaList[j] +"</a></li>");
		}		
		$(cityToggle).text(cityList[0].name);
		$(areaToggle).text(areaList[0]);
		$('#provIndex').val(provIndex);

		$(cityLi).click(function(){//城市的li单击事件
			var provIndex = $('#provIndex').val();
			if(provIndex == ""){
				provIndex = 0;
			}else{
				provIndex = $('#provIndex').val();
			}
			$(city).find('.dropdown-toggle').text($(this).text());
			$(city).find('.dropdown-toggle').attr('title',$(this).text());

			var cityIndex = $(this).index();
			var areaList = provinceList[provIndex].cityList[cityIndex].areaList;
			$(area).find('.dropdown-menu').text('');
			for(j = 0; j < areaList.length; j++){
				$(area).find('.dropdown-menu').append("<li><a href='javascript:;' title='"+ areaList[j] +"'>"+ areaList[j] +"</a></li>");
			}
			...
			$(areaLi).click(function(){//区域的li单击事件
				$(area).find('.dropdown-toggle').text($(this).text());
				$(area).find('.dropdown-toggle').attr('title',$(this).text());				
			})				
		})		
	})

	$('.dropdown-inner').click(function(){//表单传值获取
		$('#provVal').val($(provToggle).text());
		$('#cityVal').val($(cityToggle).text());
		$('#areaVal').val($(areaToggle).text());
	})
}
